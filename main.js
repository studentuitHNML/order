setInterval(function () {
  ...
  document.getElementById("updateTime").textContent = dateTime;
}, 1000);
...
const foodSelect = document.getElementById("food-slt");
...
foodSelect.addEventListener("change", function (e) {
  const tableUpdate = document.querySelector("#table-slt");
  const foodUpdate = document.querySelector("#food-slt");
  const quantity = 1;
  const getPrice = function (foodName) {
    for (let i = 0; i < combine.length; i++) {
      if (combine[i].foodName === foodName) {
  ...
  const foodSelectedValue = foodUpdate.value;
  foodCell.textContent = foodSelectedValue;
  quantityCell.textContent = quantity;
  priceCell.textContent = getPrice(foodSelectedValue);

  newRow.append(foodCell, quantityCell, priceCell);
